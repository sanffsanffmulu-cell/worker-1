...
class _ManageServerPageState extends State<ManageServerPage> {
  ...
  Future<void> _fetchVpsList() async {
    ...
    final uri = Uri.parse('https://yosh.nullxteam.fun/myServer?key=${widget.keyToken}');
    try {
  ...
  Future<void> _addVps() async {
    ...
    final uri = Uri.parse('https://yosh.nullxteam.fun/addServer');
    try {
  ...
  Future<void> _deleteVps(String host) async {
    final uri = Uri.parse('https://yosh.nullxteam.fun/delServer');
    try {
      final res = await http.post(uri, body: {
        'key': widget.keyToken,
        'host': host,
  ...
  void _showError(String msg) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text("Error", style: TextStyle(color: softWhite)),
        content: Text(msg, style: TextStyle(color: softWhite.withOpacity(0.7))),
        actions: [
          TextButton(
  ...
  void _showAddDialog() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text("Tambah VPS", style: TextStyle(color: softWhite)),
        content: Column(
        ...
        actions: [
          ...
          Container(
            ...
            child: ElevatedButton(
              ...
              child: Text("TAMBAH", style: TextStyle(color: softWhite)),
            ),
          ),
        ],
      ),
        ...
        body: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Divider(color: cyanLight),
              Expanded(
                child: isLoading
                    ? Center(child: CircularProgressIndicator(color: cyanLight))
                    : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        title: Text("${vps['host']}", style: TextStyle(color: softWhite)),
                        subtitle: Text("User: ${vps['username']}", style: TextStyle(color: softWhite.withOpacity(0.7))),
                        trailing: Container(
                          decoration: BoxDecoration(
